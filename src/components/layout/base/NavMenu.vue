  ...
  <div v-if="!isAdminRoute" class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
    <NavItem to="cart" type="icon">
      <div class="relative">
        ...
        <span v-if="cart.items.length > 0" class="absolute top-0 right-0 bg-teal-600 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">
          {{ cart.items.length }}
        </span>
      </div>
    </NavItem>
    <NavItem to="favourites" type="icon">
      <div class="relative">
        ...
        <span v-if="products.favourites.length > 0" class="absolute top-0 right-0 bg-teal-600 text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">
